"""Synthetic-injection truth test on EDF-S (OOD), v11-label anchored mega run (2026-09-07).

Same protocol as the ECDFS run, on the unseen field: truth-based transfer test for section 6.

Same protocol as injection_truth_test_q1_v11labels_mega.py (same seed and SNR
levels) with the adopted v11-label anchored head on the v11 foundation.

Injects Gaussian point sources with KNOWN sub-pixel sky positions into all 10
bands of the held-out EDF-S tiles, then runs the exact eval pipeline (classical
...
FOUNDATION = ROOT / "models/checkpoints/jaisp_v11_q1_soft/checkpoint_best.pt"
HEADS = {
    "anchored": ROOT / "models/checkpoints/latent_position_v11_anchored_v11labels/best.pt",
    "anchored_pv25": ROOT / "models/checkpoints/latent_position_v11_anchored_v11labels_patchval25/best.pt",
}
LABELS_PATH = ROOT / "data/detection_labels/centernet_q1_edfs_vissep_v11_thresh03.pt"
LABELS = torch.load(LABELS_PATH,
                    map_location="cpu", weights_only=False)
LABELS = LABELS["labels"] if "labels" in LABELS else LABELS

N_TILES = 0  # 0 = all EDF-S tiles
N_INJECT = 20
SNR_LEVELS = [5, 7, 10, 15, 30]
RNG = np.random.default_rng(20260611)
OUT_PATH = ROOT / "io/_nb09_outputs/injection_truth_results_edfs_v11labels_mega.json"
PARTIAL_PATH = ROOT / "io/_nb09_outputs/injection_truth_results_edfs_v11labels_mega.partial.json"
...
def write_records(path, records, complete=False):
    meta = {
        "foundation": str(FOUNDATION.relative_to(ROOT)),
        "heads": {k: str(v.relative_to(ROOT)) for k, v in HEADS.items()},
        "labels": str(LABELS_PATH.relative_to(ROOT)),
        "n_tiles_requested": N_TILES,
        "n_inject": N_INJECT,
...
from astrometry2.train_latent_position_v2 import LAM as _LAM
for tag, p in HEADS.items():
    ck = torch.load(p, map_location="cpu", weights_only=False)
    cfg = ck.get("config", {})
    bn_win = (cfg.get("bottleneck_window", 0) or 0) if "variant" in cfg else (cfg.get("bottleneck_window", 5) or 5)
    ...
    if ck.get("head_class") in ("BandAwareHead", "BoundedBandAwareHead", "AnchoredHead"):
        head_cls = getattr(tlp2, ck["head_class"])
        head = head_cls(
...
# ---- tiles: sorted EDF-S stems; N_TILES=0 means all ----
rdir, edir = ROOT / "data/edf_s_ood/rubin_tiles_edfs", ROOT / "data/edf_s_ood/euclid_tiles_edfs_q1"
all_stems = sorted(p.name[:-4] for p in rdir.glob("tile_*.npz"))
stems = all_stems if N_TILES == 0 else all_stems[:N_TILES]
print(f"tiles: {stems}")

if PARTIAL_PATH.exists():
    prev = json.load(open(PARTIAL_PATH))
    records = prev.get("records", [])
    print(f"resuming from {PARTIAL_PATH.relative_to(ROOT)} with {len(records)} records")
else:
    records = []

done_pairs = {(r["tile"], int(r["snr"])) for r in records}
...
for stem in stems:
    ...
    for snr in SNR_LEVELS:
        if (stem, snr) in done_pairs:
            print(f"  {stem} snr={snr}: already complete ({len(records)} records total)", flush=True)
            continue
        ...
        vis_inj_t = torch.from_numpy(np.nan_to_num(vis_inj, nan=0.0))[None, None].float().to(DEVICE)
        ...
        for band_name in [f"rubin_{b}" for b in RUBIN_BANDS] + [f"nisp_{b}" for b in NISP_BANDS]:
            ...
            for tag, head in heads.items():
                head_kwargs = {}
                if type(head).__name__ in ("BandAwareHead", "BoundedBandAwareHead", "AnchoredHead"):
                    head_kwargs["lam"] = torch.full((len(vi),), _LAM[_short],
                                                    device=DEVICE, dtype=torch.float32)
                    if type(head).__name__ == "AnchoredHead":
                        head_kwargs["vis_img"] = vis_inj_t
                with torch.no_grad():
                    out = head(
            ...
            for i, k in enumerate(vi):
                ...
                records.append(rec)
        write_records(PARTIAL_PATH, records, complete=False)
...
write_records(OUT_PATH, records, complete=True)
