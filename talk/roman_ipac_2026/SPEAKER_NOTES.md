...
Build: `pdflatex jaisp_roman_talk.tex` (×2) in this directory; figures come from `figures/` (talk versions with 1.5x labels, regenerated by `regen_talk_figures.py`), falling back to `../../paper/figures/`.
...
The why-reconstruction story now has its own slide (next). (Likely Q&A seed: "why not one big shared grid?" — resampling throws away the sharper
instrument's information.)

## 6. We didn't start here — 4:45–5:45
The credibility slide: this design was the *tenth* iteration, not a lucky guess. v1–v5 all
tried latent alignment — contrastive patch matching (collapsed on empty sky),
object-centric matching (complexity without precision), then JEPA with ever-stricter
position constraints. The decisive kill: a simple supervised CNN beat the best JEPA
features on astrometry (38 vs 47 mas) at a fraction of the cost. The turn (v6): predict
actual pixels — to reconstruct a held-out band correctly the model has no shortcut that
discards position. v7–v8 let each instrument keep its native grid; v9–v10 are refinements.
Meta-point for this audience: similarity objectives can look great and still be useless
for measurement — the negative results are the design knowledge.

## 7. Validation: reconstruction — 5:45–6:25
...
## 8. Validation: probe + routing — 6:25–7:00
...
## 11. Astrometry: position head — 8:55–10:05
...
## 12. Concordance field — 10:05–11:00
...
## 13. Transfer to a second field — 11:00–11:40
...
## 14. Roman — 11:40–13:10 (protect this slide)
