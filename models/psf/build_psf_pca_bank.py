"""Empirical per-band base ePSF bank built with photutils.EPSFBuilder.

Produces a per-band oversampled ePSF (Anderson-King iterative shift+stack
with sigma-clipping, ePSF-driven centroid refinement) from the Gaia training
stamps. The output checkpoint is loadable by
``foundation_epsf_head.load_base_epsf_bank`` and is used as the empirical
prior for ``FoundationEPSFHead`` via ``--base-epsf-mode checkpoint``.

For each band:
  1. Background-subtract each stamp via sigma-clipped median over an outer
     annulus (8-14 native px).
  2. Reject stamps whose per-stamp sigma sits outside Q25 +/- ``morpho_tol``
     (drops contaminating galaxies; galaxies are systematically wider, so
     the lower-quartile sits inside the stellar locus).
  3. Wrap each stamp as ``photutils.psf.EPSFStar`` at its image-refined
     sub-pixel centroid (``frac_xy`` from the build).
  4. ``EPSFBuilder`` builds the oversampled ePSF by sigma-clipped median
     stacking; centroids are refined using the running ePSF and the build
     is iterated until convergence.

The bank has no spatial variation -- one ePSF per band. Position-dependent
variation is captured downstream by the ``FoundationEPSFHead`` residual head.

Build:

    PYTHONPATH=models python models/psf/build_psf_pca_bank.py \\
        --train-dir   data/psf_training_gaia_pm \\
        --output-path models/checkpoints/psf_pca_bank_v10_pm/bank.pt \\
