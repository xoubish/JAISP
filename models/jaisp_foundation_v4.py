...
#
# JAISP Foundation v4 (native-res) with:
#  - signal-based InformationMap weights (SNR + gradients, device-safe)
#  - band-specific stems + shared ViT-like trunk
#  - shift-tolerant alignment loss
#  - VICReg regularization
#  - BYOL/JEPA-style Student + EMA Teacher + Predictor (to reduce instrument separation)
...
class AlignmentLoss(nn.Module):
    ...
    def forward(self, z1, z2, w1, w2, gs1, gs2):
        ...
        # average weights from the two views (resampled to token grid)
        ...
        for dy in range(-r, r + 1):
            for dx in range(-r, r + 1):
                if dy == 0 and dx == 0:
                    ...
                    z2p_ = z2g.permute(0, 3, 1, 2)  # [B,D,H,W]
...
class JAISPFoundationV4(nn.Module):
    ...
    Key idea:
      ...
      - Alignment matches student p(view1) -> teacher z(view2) and vice versa
