...
class SuperPointJEPATrainer:
    ...
    def visualize_matching(self, batch, outputs, step):
        """
        Modified to log separate panels for easier W&B side-by-side comparison.
        """
        def to_np(k): 
            val = batch[k]
        ...
        # 1. Alignment Plot (The "lines" visualization)
        ...
        # 2. Individual Heatmaps for W&B Grid Comparison View
    ...
    def train(self, epochs=100, lr=5e-5):
        opt = create_optimizer(self.model, lr)
        sched = create_scheduler(opt, 20, epochs)
        scaler = GradScaler('cuda')
        
        wandb.init(project="JAISP-SuperPoint-JEPA", name=self.wandb_name)
        ...
        for ep in range(epochs):
            ...
            sched.step()
            torch.save(self.model.state_dict(), f"stable_model_{self.wandb_name}.pt")
        wandb.finish()
...
def main():
    trainer = SuperPointJEPATrainer(
        ...
        batch_size=1,
        wandb_name="JEPA-Alignment-Run1"
    )
