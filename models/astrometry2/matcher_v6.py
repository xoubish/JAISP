...
Key differences from LocalAstrometryMatcher (matcher.py):
  ...
  Rubin encoder:
    - Uses per-band BandStem CNNs from the frozen v6 foundation model.
    - Each band channel processed through its own frozen stem (physically
      meaningful: each band has its own noise/PSF characteristics), then
...
# Make sure the parent models/ dir is on the path so we can import v6
...
class V6RubinEncoder(nn.Module):
    """
    Multi-band Rubin patch encoder that reuses frozen v6 BandStem weights.

    Input : [B, n_bands, H, W]  pre-noise-normalized patches, bands stacked
                                 in RUBIN_BAND_ORDER (u, g, r, i, z, y)
    Output: [B, out_ch, H, W]

    Pipeline:
      1. Split stacked input back into per-band tensors [B, 1, H, W]
      2. Each band → its frozen BandStem CNN (Conv5→GN→GELU→Conv3→GN→GELU)
         The normalization step (image/rms) is skipped because patches are
         already noise-normalized by the astrometry2 dataset loader.
      3. Mean across bands → [B, stem_ch, H, W]
      4. Trainable ConvNeXt adapter blocks → [B, out_ch, H, W]
         These refine the frozen features toward astrometry-relevant signals
         without destroying what the foundation model learned.
    """
    ...
    def __init__(
        ...
        stem_ch = v6_model.encoder.stem_ch   # typically 64

        # Extract the CNN part (.net) from each band's BandStem.
        # We do NOT reuse the normalization (image/rms) because astrometry2
        # patches are already in noise-normalized units.
        band_names = [f'rubin_{b}' for b in RUBIN_BAND_ORDER[:n_input_bands]]
        self.band_stems = nn.ModuleList([
            v6_model.encoder.stems[name].net   # nn.Sequential: Conv→GN→GELU→Conv→GN→GELU
        ...
        # Trainable adapter: ConvNeXt blocks at stem_ch, then optional projection
    ...
    def forward(self, rubin_patch: torch.Tensor) -> torch.Tensor:
        """
        rubin_patch: [B, n_bands, H, W] pre-normalized
        returns:     [B, out_ch, H, W]
        """
        n = min(rubin_patch.shape[1], len(self.band_stems))
        feats = [
            self.band_stems[i](rubin_patch[:, i:i+1, :, :])   # [B, stem_ch, H, W]
            for i in range(n)
        ]
        x = torch.stack(feats, dim=0).mean(dim=0)   # [B, stem_ch, H, W]
        return self.adapter(x)                        # [B, out_ch, H, W]
...
class V6AstrometryMatcher(nn.Module):
    ...
    Jacobian       : pixel shift → sky arcsec (identical to astrometry2)

    API is intentionally identical to LocalAstrometryMatcher so it can be
    swapped in to the existing training / inference scripts with minimal changes.
    """
    ...
    def __init__(
        ...
        # ---- Encoders ------------------------------------------------------
        self.rubin_encoder = V6RubinEncoder(
            v6_model=v6_model,
    ...
    # ------------------------------------------------------------------ #
    # Internals (identical logic to LocalAstrometryMatcher)               #
    # ------------------------------------------------------------------ #
    ...
    def predict_all_bands(
        ...
        """Encode once, predict for every target band. Efficient at inference."""
...
def load_v6_matcher(
    ...
    ckpt = torch.load(v6_checkpoint, map_location='cpu')
    cfg  = ckpt.get('config', {})

    v6 = JAISPFoundationV6(
    ...
    v6.load_state_dict(ckpt['model'])
    v6.eval()

    matcher = V6AstrometryMatcher(
        v6_model        = v6,
    ...
    print(f'V6AstrometryMatcher: {n_total/1e6:.1f}M total, {n_trainable/1e6:.1f}M trainable')
    print(f'  (frozen v6 stems: {(n_total - n_trainable)/1e6:.1f}M)')
