"""[v2 — nb23 clamp-fix test: brightness-balanced sampling + optional asinh raw side-channel]
...
def build_tile_batch(
    ...
    device: torch.device,
    vis_snr: np.ndarray = None,
    balanced: bool = False,
    bright_snr: float = 100.0,
    bright_repeat: int = 8,
    force_snr: float = 40.0,
    ...
    # Subsample if too many sources; balanced mode protects and oversamples bright sources.
    if balanced and vis_snr is not None and len(vis_snr) == len(vis_xy):
        snr = np.asarray(vis_snr, dtype=np.float32)
        if N > max_sources:
            forced = np.where(snr > force_snr)[0]
            rest = np.where(snr <= force_snr)[0]
            n_rest = max(0, max_sources - len(forced))
            pick = rng.choice(rest, min(n_rest, len(rest)), replace=False) if len(rest) else np.array([], dtype=int)
            idx = np.concatenate([forced, pick]).astype(int)
            vis_xy = vis_xy[idx]; snr = snr[idx]
        rep = np.where(snr > bright_snr)[0]
        if len(rep):
            vis_xy = np.concatenate([vis_xy, np.repeat(vis_xy[rep], bright_repeat, axis=0)], axis=0)
        N = vis_xy.shape[0]
    elif N > max_sources:
...
def run_epoch(
    ...
    def _get_tile_data(idx):
        ...
        if dual_gpu:
            ...
            vis_stem = enc_out['vis_stem']
            fused_hw = enc_out['fused_hw']
            vis_soft = (soft_snr_map(img_t['euclid_VIS'], rms_t['euclid_VIS'])
                        if getattr(args, 'raw_channel', 0) else None)
            del img_t, rms_t
            try:
                edata = np.load(euclid_path, allow_pickle=True)
            ...
            return {
                ...
                'vis_snr': vis_snr, 'vis_soft': vis_soft,
    ...
    for i, idx in enumerate(order):
        ...
        vis_soft   = tile_data.get('vis_soft')
        ...
        batch = build_tile_batch(
            ...
            rng=rng,
            device=device,
            vis_snr=tile_data.get('vis_snr'),
            balanced=bool(getattr(args, 'balanced', 0)),
            bright_snr=args.bright_snr,
            bright_repeat=args.bright_repeat,
        )
        if batch is None:
            continue
        ...
        with torch.set_grad_enabled(is_train):
            _kw = {'vis_soft': vis_soft} if getattr(args, 'raw_channel', 0) else {}
            out = head(
                bottleneck, vis_stem,
                batch['positions'],
                batch['pixel_to_sky'],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--limit-tiles', type=int, default=0,
                   help='debug: cap the number of train/val tiles')
    p.add_argument('--raw-channel', type=int, default=0,
                   help='1 = LatentPositionHeadV2 with asinh raw VIS window side-channel')
    p.add_argument('--raw-out', type=int, default=32)
    p.add_argument('--balanced', type=int, default=0,
                   help='1 = protect + oversample bright sources in per-tile sampling')
    p.add_argument('--bright-snr', type=float, default=100.0)
    p.add_argument('--bright-repeat', type=int, default=8)
...
def train(args):
    ...
    if getattr(args, 'limit_tiles', 0):
        train_pairs = train_pairs[:args.limit_tiles]
        val_pairs = val_pairs[:max(2, args.limit_tiles // 6)]
    ...
    if getattr(args, 'raw_channel', 0):
        bw = args.bottleneck_window if args.bottleneck_window > 0 else 11
        head = LatentPositionHeadV2(
            hidden_ch=256, stem_ch=64,
            bottleneck_out=args.bottleneck_out, stem_out=args.stem_out,
            mlp_hidden=args.mlp_hidden, bottleneck_window=bw,
            stem_window=args.stem_window, fused_pixel_scale=0.4,
            vis_pixel_scale=0.1, raw_out=args.raw_out,
        ).to(device)
        n_tr = sum(p_.numel() for p_ in head.parameters())
        print(f'Using LatentPositionHeadV2 (asinh raw side-channel, raw_out={args.raw_out}): '
              f'{n_tr/1e6:.2f}M params')
        args.dual_gpu = False
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if wandb_run:
            ...
            if epoch % args.vis_every == 0 and preview_pairs and not getattr(args, 'raw_channel', 0):
