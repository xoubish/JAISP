    ...
    with (out / 'monitor.lock').open('a') as lock:
        ...
        try:
            while True:
                ...
                # Epochs take longer than W&B's stale-run timeout. This CPU
                # monitor intentionally disables system statistics, so send a
                # small liveness metric while waiting for new validation files.
                # No validation metric is repeated and no epoch is invented.
                now = time.monotonic()
                if watch and now - last_heartbeat >= 60:
                    run.log({'monitor/heartbeat_utc_seconds': time.time(),
                             'monitor/latest_shared_epoch': latest})
                    last_heartbeat = now
