...
A liveness scalar is sent once per minute while waiting. Validation arrives
roughly every nine minutes, which can otherwise make W&B mark this quiet CPU
monitor as crashed despite its process remaining alive. Liveness updates do
not repeat validation measurements or increment the validation epoch.
