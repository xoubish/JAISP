"""Training script for JAISP Foundation v10 — standalone trainer.

Self-contained consolidation of the v7 training infrastructure, v8 random
cropping, v9 adversarial cross-instrument masking, and v10 Charbonnier-with-
core-L2 loss shaping. Imports only the standalone ``jaisp_foundation_v10``
and ``jaisp_dataset_v10`` modules — no inheritance chain, no older-version
dependencies.

V10 architecture and loss are baked into ``JAISPFoundationV10`` directly:
  - Symmetric concat fusion (``rubin_concat=True``).
  - Charbonnier per-pixel loss (sqrt(diff^2 + eps^2)) — sharp source cores
    without sacrificing noise robustness.
  - Optional core-L2 term on high-info pixels.

Sampling pipeline per training step:
  1. Spatially-consistent random crop across all bands (Rubin pixel coords).
  2. Phase B mixed-resolution split (or within-instrument Phase A as a fallback
     when ``cross_instrument_prob`` < 1 or the tile lacks Euclid).
  3. Adversarial drop: with prob ``p_adversarial`` mask 1-2 wavelength-adjacent
     within-instrument neighbours of the target.
...
Multi-GPU::
    torchrun --nproc_per_node=2 train_jaisp_foundation_v10.py [...same args...]
"""
...
# ============================================================
# Within-instrument Phase A split (used when Euclid is unavailable
# or when the trainer randomly picks Phase A instead of Phase B).
# ============================================================

def _phase_a_within_rubin(sample: dict, rng: np.random.RandomState, n_targets: int) -> Optional[dict]:
    """Mask one Rubin band given the rest. Used when Euclid is unavailable."""
...
def _phase_b_mixed(sample: dict, rng: np.random.RandomState, n_targets: int) -> Optional[dict]:
    """Pool Rubin + Euclid (each at native resolution); hold out n_targets bands."""
...
class JAISPTrainerV10:
    """Standalone v10 trainer.

    Builds a ``JAISPFoundationV10`` directly with all v10 settings
    (``rubin_concat=True``, Charbonnier loss, core-L2 term). No legacy
    inheritance.
    """
    ...
    def __init__(
        ...
        rubin_dir: str,
        euclid_dir: str,
        output_dir: str = "./checkpoints/jaisp_v10",
        # Architecture
        stem_ch: int = 64,
        hidden_ch: int = 256,
        ...
        wandb_project: str = "JAISP-Foundation-v10",
        wandb_name: str = None,
        # Misc
        seed: int = 42,
        ...
        self.n_targets = int(n_targets_per_step)
        ...
        self.vis_every = int(vis_every_n_epochs)
        ...
        with suppress_stdout(not self.is_main_process):
            self.train_dataset = JAISPDatasetV10(
                rubin_dir=rubin_dir,
        ...
        # Validation must be deterministic and augmentation-free.
        ...
        self.train_loader, self.train_sampler = self._make_subset_loader(
            self.train_dataset, self.train_indices, shuffle=True, generator_seed=self.seed,
        )
        self.val_loader = self._make_subset_loader(
            self.val_dataset, self.val_indices, shuffle=False,
        )
        if self.is_main_process:
        ...
        # --- model ----------------------------------------------------
        if self.is_main_process:
            print("Initializing JAISPFoundationV10...")
        with suppress_stdout(not self.is_main_process):
            base_model = JAISPFoundationV10(
        ...
        self.use_wandb = (
            wandb is not None and self.is_main_process
            and not wandb_disabled and wandb_mode != "disabled"
        )
        if self.use_wandb:
            wandb_kwargs = dict(
                project=wandb_project,
                name=wandb_name or f"v10_h{hidden_ch}_d{transformer_depth}_l2{core_l2_weight}",
                config={
                    "stem_ch": stem_ch, "hidden_ch": hidden_ch,
                    "blocks_per_stage": blocks_per_stage,
                    "transformer_depth": transformer_depth,
                    "transformer_heads": transformer_heads,
                    "fused_pixel_scale_arcsec": fused_pixel_scale_arcsec,
                    ...
                    "crop_size_rubin": crop_size_rubin,
                    "cross_instrument_prob": cross_instrument_prob,
                    "p_adversarial": p_adversarial,
        ...
        # Config dict saved into checkpoints — load_foundation reads these
        # markers to detect v10 (loss_type='charbonnier', core_l2_weight>0,
        # rubin_concat=True).
        self.config = {
            "band_names": ALL_BANDS,
            "stem_ch": stem_ch,
    ...
    def _make_subset_loader(self, dataset, indices, shuffle: bool, generator_seed: int = None):
        subset = Subset(dataset, list(indices))
        generator = None
        sampler = None
        if self.distributed and shuffle:
            sampler = DistributedSampler(
                subset, num_replicas=self.world_size, rank=self.rank,
                shuffle=True,
    ...
    def _prepare_batch(
        ...
        force_phase_b: bool = False,
    ) -> Optional[dict]:
        """Random crop → Phase-B (cross-instrument) split → adversarial drop.

        Falls back to within-Rubin Phase A when the tile lacks Euclid coverage
        or when ``cross_instrument_prob`` randomly selects Phase A.
        """
        ...
        if use_phase_b:
            split = _phase_b_mixed(sample, rng, n_targets=self.n_targets)
            if split is not None and self.p_adversarial > 0.0:
            ...
            split = _phase_a_within_rubin(sample, rng, n_targets=self.n_targets)
        if split is None:
            return None
    ...
    def _train_epoch(self, epoch: int) -> dict:
        ...
        band_norm_losses = defaultdict(list)
        n_steps = 0
        ...
        # Flush any remaining accumulation steps at end of epoch.
        if accum_count > 0:
            if self.grad_clip > 0:
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.grad_clip)
            self.optimizer.step()
    ...
    def _visualize(self, epoch: int) -> None:
        ...
        for target_band in ordered_bands:
            ...
            panel_data.append({
                "band": target_band, "truth": truth, "pred": pred,
                "resid": resid, "info": info,
                "corr": corr, "mae": mae, "std_ratio": std_ratio,
            })
        ...
        if panel_data:
            ...
            for col, panel in enumerate(panel_data):
                truth = panel["truth"]; pred = panel["pred"]
                resid = panel["resid"]; info = panel["info"]
                ...
                if np.isfinite(panel["corr"]):
                    title = (
                        f"{title}\n"
                        f"r={panel['corr']:.2f}  mae={panel['mae']:.2f}  std={panel['std_ratio']:.2f}"
                    )
            ...
            fig.suptitle(
                f"v10 reconstruction diagnostics | tile={tile_id} | bands={len(panel_data)} | fused={fused_txt}",
                fontsize=12, y=0.995,
            )
    ...
    def load_checkpoint(self, path: str, weights_only: bool = False) -> int:
        ...
        # Weights-only resume = warm-start a NEW training run from the loaded
        # weights: don't inherit the prior best_val_loss / step counter / epoch.
        if weights_only:
            self.best_val_loss = float("inf")
            self.global_step = 0
        else:
    ...
    def train(self, resume_from: str = None, resume_weights_only: bool = False) -> None:
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            self.best_val_loss = self._broadcast_scalar(
                self.best_val_loss if self.is_main_process else 0.0
            )
            self._barrier()
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Train JAISP Foundation v10 (standalone trainer: "
                    "concat fusion + adversarial masking + Charbonnier + core-L2)."
    )
    p.add_argument("--rubin_dir", required=True)
    p.add_argument("--euclid_dir", required=True)
    p.add_argument("--output_dir", default="./checkpoints/jaisp_v10")
    p.add_argument("--resume", default=None)
    p.add_argument(
        "--resume_weights_only", action="store_true",
        help="Resume model weights and split metadata, but reinit optimizer/scheduler/RNG.",
    )
    # Architecture
    ...
    p.add_argument("--fused_pixel_scale_arcsec", type=float, default=0.4)
    # Loss-shaping
    p.add_argument("--loss_type", type=str, default="charbonnier", choices=["l1", "charbonnier"])
    ...
    p.add_argument("--crop_size_rubin", type=int, default=256)
    p.add_argument("--cross_instrument_prob", type=float, default=1.0)
    p.add_argument("--p_adversarial", type=float, default=0.25)
    p.add_argument("--n_extra_max", type=int, default=2)
    # Validation / logging
    p.add_argument("--val_fraction", type=float, default=0.05)
    p.add_argument("--vis_every_n_epochs", type=int, default=2)
    p.add_argument("--wandb_project", default="JAISP-Foundation-v10")
