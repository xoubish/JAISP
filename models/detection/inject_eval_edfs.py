...
def main():
    ...
    stems = sorted(p.name.replace('_euclid.npz', '')
                   for p in EUCLID.glob('tile_*_euclid.npz'))
    # keep only tiles overlapping the MER footprint (EDF-S catalog covers one Euclid TILE)
    ra_lo, ra_hi = mer['cRA'].min(), mer['cRA'].max()
    de_lo, de_hi = mer['cDEC'].min(), mer['cDEC'].max()
    kept = []
    for st in stems:
        try:
            ed = np.load(EUCLID / f'{st}_euclid.npz', allow_pickle=True)
            from detection.validation_utils import safe_header_from_card_string as _sh
            from astropy.wcs import WCS as _W
            w = _W(_sh(ed['wcs_VIS'].item()))
            cra, cde = w.all_pix2world(512, 512, 0)
            if ra_lo - 0.02 < float(cra) < ra_hi + 0.02 and de_lo - 0.02 < float(cde) < de_hi + 0.02:
                kept.append(st)
        except Exception:
            continue
    stems = kept[:N_TILES]
