...
**Leakage caveat (2026-06-10 audit).** These eval numbers cover all 790 tiles, ~85% of which the head trained on, and the tile-level seed-42 split does not hold out *sky*: because adjacent tiles overlap 50%, ~95% of val-tile sources have duplicates in training tiles — and those duplicates carry *identical* labels, since overlapping tiles are crops of the same mosaic (see Tiling and Overlap). Train/val statistics are consequently indistinguishable (val pooled 62.8→25.6 MAE vs train 62.6→25.3). On the truly-unseen subset (val-tile sources with no train-tile duplicate, N=5,111), head medians at fixed SNR are 3-4× worse than the headline (e.g. 55 vs 12 mas at SNR 7-10), converging only above SNR≈30. The honest read: the head genuinely generalizes a ~40-50% residual reduction; the headline 75-77% reduction partially reflects per-source label-noise fitting on seen sources. Publication-grade numbers require a spatially disjoint split (hold out a full patch and retrain).

##### Ongoing (started 2026-06-10): patch-disjoint baseline and the label-noise program
...
Key features:
- **Current v10 default**: Gaussian-centroid control on CenterNet threshold-0.3 labels improves archive eval medians **48.5 -> 11.3 mas** and deduped Notebook 08 medians **54.1 -> 13.2 mas** (in-sample-dominated; see the leakage caveat in the eval section — clean generalization is ~40-50% reduction at fixed SNR).
...
| **Latent position head v10 Gaussian default** | `models/checkpoints/latent_position_v10_no_psf/best.pt` | Current per-object astrometry correction. Uses v10 foundation features, CenterNet `conf=0.3` labels, Gaussian-fit photon centroids, and `bottleneck_window=5`; eval median **48.5 -> 11.3 mas** before dedup and **54.1 -> 13.2 mas** on the deduped Notebook 08 cache (in-sample-dominated; clean generalization ~40-50% at fixed SNR — see eval leakage caveat). Matched-source comparison shows parity with the v8 head (8.6 vs 8.6 mas median on identical sources). |
...
**Open milestones**
- **Patch-disjoint astrometry baseline (in progress, 2026-06-10)**: `latent_position_v10_patchval25` retrains the production head with patch 25 fully held out (see § "Ongoing: patch-disjoint baseline and the label-noise program"). Deliverable: the first leakage-free faint-end number, plus the measured gap vs the production head on identical tiles.
- **Label-noise program for faint anchors**: joint multi-band centroid targets → heteroscedastic (offset, sigma) head → sigma-weighted duplicate-aware field fitting. Success gate: the notebook-08 sparse-field test (head-corrected faint anchors reproducing the classical-anchor field to a few mas) passes.
- **Label-free truth metrics**: synthetic source injection for the faint end (adapt notebook 06 injection machinery to astrometry); Gaia-PM absolute validation at the bright end. Needed because overlap duplicates are pixel-identical and cannot serve as repeat measurements.
- **Out-of-distribution evaluation** (see Motivation § "Why a Foundation Model"). All current metrics are on ECDFS tract5063. We have not yet evaluated on a non-ECDFS field. Until we do, we cannot measure the foundation's actual value proposition -- transfer without retraining. Highest-leverage next experiment after astrometry settles: download ~50 tiles from EDF-North, run the latent-head eval + residual-field QA without any retraining, compare the MAE to the ECDFS numbers. Per the 2026-06-10 audit this is also the discriminating test for any future foundation version, since in-distribution probes and downstream metrics show v8/v9/v10 as equivalent.
