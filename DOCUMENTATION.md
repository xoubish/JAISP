...
| Detection | Current v10 checkpoints: `checkpoints/centernet_v10_uncertain_synth_r2/centernet_best.pt` and `checkpoints/stem_centernet_v10_uncertain_synth_r2/stem_centernet_best.pt`; retained v8 cache: `data/detection_labels/centernet_v8_r2_790.pt` | Notebook 06 treats the cleaner VIS-only classical reference as the decision baseline, not the SEP-assisted variant. Inference should run with spike veto off (`--spike_veto_radius 0 --spike_veto_width 0`): the veto was suppressing real bright/large objects (note the argparse *defaults* still have the veto on; the flags must be passed). Per the seeded 10-tile sweep (`io/_nb06_outputs/sweep_seeded.json`, 2026-06-10): StemCenterNet `conf=0.55` is exactly optimal against the VIS-only reference; CenterNet `conf=0.30` is a deliberate recall-biased choice — against the VIS-only reference the pooled F1 optimum is `0.40` (0.30 costs ~1.4-1.9 pp F1 for ~5 pp extra recall), and 0.30 is optimal only against the SEP-assisted reference. `run_centernet_detections.py` auto-loads both fused and stem detector checkpoints. |
| Astrometry | Current default: `models/checkpoints/latent_position_v10_no_psf/best.pt`; ablation: `models/checkpoints/latent_position_v10_epsf_centroid/best.pt`; retained baseline: `models/checkpoints/latent_position_v8_no_psf/best.pt` | The v10 rerun used exported CenterNet anchors at threshold 0.3 plus the full `data/cached_features_v10_warmstart` bottleneck cache. The Gaussian-centroid control is the current winner: eval raw/head MAE **62.6 -> 25.4 mas** and median **48.5 -> 11.3 mas** on 693,564 source-band measurements. The FoundationEPSFHead centroid ablation produced more anchors but worse residuals (**76.0 -> 39.2 mas** MAE; median **70.7 -> 26.7 mas**), so it is not the default astrometry path. **Audit verdict (2026-06-11)**: the patch-disjoint retrain (`latent_position_v10_patchval25`, patch 25 fully held out) reproduces these numbers on never-seen sky in every band and SNR bin (pooled median 10.7 vs 11.0 mas on identical sources) — no leakage inflation. Interpretive caveat: at the faint end the residual-vs-label metric measures emulation of the classical centroid; the injection truth test (2026-06-11) measured actual faint-end accuracy at **18.5 mas vs truth at SNR 5** (vs 47 mas classical band centroid) — a real ~2.5x gain, mechanism = VIS-localization transfer. The joint-label A/B (`latent_position_v10_patchval25_joint`) falsified the label-limited hypothesis — the head has an intrinsic ~17-18 mas faint-end ceiling — so the classical joint 10-band fitter (6.7 mas at SNR 5) is the production position instrument; its anchor archive (`anchors_joint_canonical_790.npz`) yields ~5x more reproducible field fits and is Gaia-validated to 4.4-4.8 mas absolute at G<18.5. |
...
#### Matched-protocol probe: v7 vs v10, and representation geometry (2026-06-28, paper Figure 4)

The v8/v9/v10 equivalence above is *within the 0.4"/px concat lineage*. A separate question is whether the architecture/resolution change from **v7** (0.8"/px mixed-resolution, mean Rubin fusion) to **v10** (0.4"/px fine-scale, concat fusion) actually improved the representation — the original motivation for the upgrade. v7 had never been probed. Per the protocol rule (a frozen probe must be run on **all** checkpoints compared), the Diag-2b source-centered ridge+MLP probe was extracted from notebook 05 into a standalone parameterised runner and run on v7 and v10 identically.

- **Script**: `scratchpad/run_probe_v7_v10.py` (6-tile leave-one-tile-out, the documented protocol; reproduces the original v10 numbers to ~0.02, validating the reimplementation) and `scratchpad/run_probe_v7_v10_big.py` (scaled to ~60 tiles / ~1100 sources, 6-fold grouped spatial hold-out, to silence small-N objections). `load_foundation` handles both classes; feature extraction is scale-aware via `model.encoder.fused_pixel_scale_arcsec`. Sidecars: `io/_nb13_outputs/cross_instrument_summary_{v7,v10}_matched.json` (and `_matched_big.json`).
- **Qualitative finding (robust)**: in v7 the deep Euclid bands are only weakly and *nonlinearly* encoded (NISP J: linear ridge R²≈0.12, rising to ≈0.58 only with the MLP); in v10 they become strongly and *linearly* decodable (J: ridge R²≈0.84). The Rubin ugrizy bands remain present-but-largely-nonlinear in both. So the fine-scale recipe reorganized cross-instrument information into a near-linearly-accessible geometry.
- **Mutual-information framing**: the probe R² is a variational lower bound on the MI between the representation and each band, I ≳ −½·log₂(1−R²) bits for a Gaussian target (J: v7 ≈0.09 bits → v10 ≈1.3 bits linearly). The plotted markers read directly as recoverable information content.
- **Effective dimensionality cross-check (rules out the trivial explanation)**: the v10 linear-probe jump is *not* "v10 uses more of its latent." On the full bottleneck (15k positions/checkpoint, equal N), the effective dimensionality is essentially unchanged: participation ratio 4.9 (v7) vs 5.2 (v10) on the covariance (7.9 vs 8.2 on the correlation matrix); spectral-entropy dimension 10.3/15.4 (v7) vs 11.7/18.5 (v10) of 256; ~75% of variance in the top-5 PCs for both. Both bottlenecks are low-dimensional (~5–18 effective dims); v10 packs information of comparable dimensionality into a more linearly decodable form. Script `scratchpad/run_effdim_v7_v10.py`, data `io/_nb13_outputs/bottleneck_effdim_v7_v10.json`. (Raw Shannon entropy of the 256-d latent is deliberately avoided — undersampled and unreliable at these source counts.)
- This is the basis for **paper Figure 4** (`paper_figures/figure_4_probe.ipynb` → `paper/figures/fig4_probe.png`): a two-panel v7 | v10 dumbbell plot (open = linear ridge, filled = nonlinear MLP, line = nonlinear gain).
...
**Status**: trained via warm-start from v9. Current checkpoint: `models/checkpoints/jaisp_v10_warmstart/checkpoint_best.pt`. This is the frozen foundation used by the current v10 detection export, the v10 Gaussian/ePSF astrometry comparison, and the Gaia/V10 ePSF-head checkpoints.
...
| **CenterNet v10 (current notebook choice)** | `checkpoints/centernet_v10_uncertain_synth_r2/centernet_best.pt` | Fused-bottleneck CenterNet on V10 warm-start features. Notebook 06 selected `conf_threshold=0.30` for inference (pooled 5-tile F1 peak is `0.35`, but `0.30` keeps more sources on the same plateau). Export with spike veto off: `--spike_veto_radius 0 --spike_veto_width 0`. |
...
| **StemCenterNet v10 (current notebook choice)** | `checkpoints/stem_centernet_v10_uncertain_synth_r2/stem_centernet_best.pt` | Native-resolution stem detector from the uncertain-ignore + synthetic-source round-2 line. Notebook 06 selected `conf_threshold=0.55`, spike veto off — confirmed exactly optimal against the VIS-only reference in the seeded 10-tile sweep (`io/_nb06_outputs/sweep_seeded.json`); the older pooled recall `89.7%` / precision `83.8%` / F1 `86.7%` figures were measured against the SEP-assisted reference on 5 unsaved tiles. The batch exporter auto-loads Stem checkpoints too. |
...
| **Latent position head v10 Gaussian default** | `models/checkpoints/latent_position_v10_no_psf/best.pt` | Current per-object astrometry correction. Uses v10 foundation features, CenterNet `conf=0.3` labels, Gaussian-fit photon centroids, and `bottleneck_window=5`; eval median **48.5 -> 11.3 mas** before dedup and **54.1 -> 13.2 mas** on the deduped Notebook 08 cache (replicate on held-out sky, 2026-06-11 patch-disjoint result; faint-end label residuals = centroid emulation). Matched-source comparison shows parity with the v8 head (8.6 vs 8.6 mas median on identical sources). |
...
**Foundation models**
- **V10 warm-start (current production)**: `models/checkpoints/jaisp_v10_warmstart/checkpoint_best.pt` -- v9 concat fusion with Charbonnier/core-L2 reconstruction loss. Used by the current detection export, PSF-head checkpoints, and v10 astrometry products. Standalone module: `models/jaisp_foundation_v10.py`.
...
- **Out-of-distribution evaluation -- FIRST RESULT (2026-06-18, EDF-S)** (see Motivation § "Why a Foundation Model"). Until now all metrics were on ECDFS tract5063; this is the first non-ECDFS field, run on the frozen v10 stack with **no retraining**. Field: EDF-S (tract 2394), 72 paired tiles in `data/edf_s_ood/`; pipeline and analysis in `io/15_ood_evaluation_edfs.ipynb`. **Important caveat on this run:** the EDF-S Euclid tiles carry no `var_*` maps, so VIS/NISP RMS used a MAD-based fallback (`_rms_from_var_or_image`), patched into `run_centernet_detections.py`, `astrometry2/joint_centroid.build_band_stack`, and `astrometry2/export_joint_anchors.py` (NISP branch); Rubin `var` is present and used directly. These are stopgaps to be removed once real-var tiles arrive.
  ...
  - **Resolved by incoming data:** real-var EDF-S tiles (re-run without the MAD fallback -> settles Caveat 1); the MER Euclid catalog (ground-truth detection completeness/purity -> settles whether the bright-star suppression loses real sources or noise). Per the 2026-06-10 audit this OOD test is also the discriminating experiment for any future foundation version, since in-distribution probes and downstream metrics show v8/v9/v10 as equivalent.
