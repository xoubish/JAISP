...
**Revised next steps (supersedes the lever ordering below):**
1. **Masked-band evaluation (cheap, decisive, uses existing data).** Re-evaluate the head with the target band *withheld from the encoder input* (the MAE foundation zero-fills missing band slots by design). The prediction must then come from the other 9 bands, so its residual against the classical label decomposes as truth-error (+) label-noise in quadrature — making the truth-error extractable statistically. This directly measures the actual use case: imputing band positions for sources too faint to centroid classically.
2. **Synthetic injection truth test** (adapt notebook 06 injection machinery): inject sources with known positions, measure head error against truth — the gold standard, immune to the emulation degeneracy.
3. **Gaia-PM absolute check** at the bright end.
4. The label-noise levers (joint multi-band centroid targets, heteroscedastic head) remain relevant for *training*, but their value should be judged by the truth metrics above, not by label residuals.
