...
| Foundation | `models/checkpoints/jaisp_v10_q1_long/checkpoint_best.pt` (**current production**, v10 architecture retrained on Q1, 2026-07-04, best val 7.1014); DR1-era `jaisp_v10_warmstart` and `jaisp_v8_fine` retained for completed baselines | v10 architecture (`models/jaisp_foundation_v10.py`): v9 concat fusion + Charbonnier loss + core-L2 weighting, fused scale 0.4"/px. Q1 feature caches: `data/cached_features_v10_q1/` (790 ECDFS) and `.../v10_q1_edfs/` (72 EDF-S); the DR1 full cache `cached_features_v10_warmstart/` was deleted. **Fused-scale ablation (DONE 2026-07-09)**: matched 0.2"/px vs 0.4"/px pair = equivalent on matched-crop reconstruction + probe → grid refinement buys nothing; v2 direction is multi-scale head readout (see the fused-scale ablation section + design note). |
| Detection | **`checkpoints/q1_detection/centernet_vis_sep.pt`** at `conf=0.30`, spike veto off (Q1 production, selected 2026-07-05 bake-off); exports `data/detection_labels/centernet_q1_790_vissep_thresh03.pt` (224 det/tile) and `centernet_q1_edfs_vissep_thresh03.pt` (277 det/tile) | Q1 bake-off on 108 held-out tiles: F1 93.9 (completeness 93.3, purity 94.5). Injection (2026-07-14 donor-morphology audit, `io/22`): point-source 50% depth **26.6 all-band vs 26.1 VIS-only → +0.45 mag fusion gain** (star-like donors, C≥0.65, 19.5–22.5, `--rvis 30 --donor-conc 0.65 --donor-faint 22.5`; metrics `injection_metrics_r30_star{,_deep}.json`); legacy 25.04/24.59 = extended-morphology completeness (donors were compact galaxies, median C≈0.31) — same gain; dimming validated by real sources at native brightness (98–99% at 23–24.5); stem control +0.0; NISP-only peaks ≈30%; Rubin-only ≈0 is DR1-era evidence, not re-verified on Q1. Round-2 self-training rejected (`q1_detection_r2/`: F1 93.7, depth 24.90 — no gain). MER recovery: purity 0.944, ≥95% at S/N≥7.9. Spike-veto-off rule carries over from DR1 (argparse defaults still have the veto ON; flags must be passed). DR1-era `centernet_v10_uncertain_synth_r2` / `stem_centernet_v10_uncertain_synth_r2` retained as history. |
...
**RESULT (2026-07-09): EQUIVALENCE — one more rung down buys nothing measurable in-distribution.** Both runs completed 300/300 epochs (interrupted once at epochs 113/119 by a session restart and resumed full-state — which exposed and fixed a latent RNG-restore bug in `load_checkpoint`, see the code comment there; runs relaunched `setsid`-detached). Best val: A 5.7131 (epoch 256), B 6.9504 (epoch 222) — B beating production's 7.1014 validates the from-scratch control; the A/B val losses themselves are NOT comparable (different crop statistics). The matched evaluation (`io/20_fused_scale_ablation_eval.ipynb`, outputs in `io/_nb20_outputs/`) ran both checkpoints on identical seeded crops of the 39 shared held-out val tiles at both crop geometries:
- **Masked-band reconstruction** (Fig-3 metric, per-band median r): statistically identical — all-band median 0.8335 vs 0.8335 at crop 128, 0.8834 vs 0.8843 at crop 256; VIS 0.63/0.72 for both models; no band differs beyond the third decimal.
- **Decodability probe** (frozen Fig-4 protocol, MLP R²): equivalent at the frozen 256-px geometry (A Rubin/Euclid 0.930/0.899 vs B 0.927/0.891, both matching production). At 128 px A leads on Rubin (0.814 vs 0.695) but that geometry is off-distribution for B — it mainly shows each model is strongest at its home crop size.
- **Interpretation**: exactly mirrors the v8≈v9≈v10 experience — in-distribution reconstruction and probes cannot separate foundations of this family. The v7→v8 fused-scale gain showed up in *downstream astrometry*, not recon. The decisive tests for 0.2"/px remain (a) the latent-position-head faint-end ceiling (~17-18 mas) — which requires a windowed/stitched feature-cache path, since full-tile encoding at 0.2"/px is 262K tokens and infeasible; and (b) the EDF-S zero-shot protocol. Neither run yet; the 0.4"/px production foundation stands.
**Where a v2 architecture should look (design note, 2026-07-09).** This result redirects the "finer bottleneck" instinct. The single-scale fused grid is the architecture's one real tax, and the evidence now triangulates on *multi-scale readout*, not grid refinement:
1. The position head already bypasses the bottleneck to reach the 0.1"/px VIS stem for fine sampling — an ad-hoc confession that no single fused scale serves both context (coarse) and precision (fine). Every future precision head (shapes especially) will likely need the same bypass, at which point "heads read the bottleneck" is qualified out of existence one head at a time.
2. The detection head misses the largest diffuse galaxies because a 0.4" grid cannot concentrate their flux into a peak — a readout-scale problem, and notably one where *coarser*, not finer, context is what's missing.
3. This ablation shows the encoder's information content is invariant to the fused scale at matched compute — so the fix for both problems above cannot be "retrain the encoder finer."
The v2 direction is therefore to expose the existing encoder pyramid to heads as a first-class multi-scale interface — the decoder already consumes scale-matched skip connections (`build_target_skips` routes each stage to the closest-scale pyramid level across both streams); heads should get the same API instead of each hand-rolling its own stem bypass. Paper §3/§4/§7 were updated 2026-07-09 to reflect this framing (fused scale = measured choice; next step = multi-scale readout, not finer grid). The two measurements that could still change this picture are unrun: the position-head faint-end ceiling on 0.2" features (blocked on a stitched/windowed feature cache) and the EDF-S zero-shot discriminator.

A related but NOT-run idea recorded for later: `--transformer_depth 0` (conv-only fusion, zero-code ablation of the bottleneck transformer) against control B — answers the "does full self-attention over 16K tokens earn its cost" referee question.
...
- **Injection depth** — SUPERSEDED 2026-07-14 by the donor-morphology audit (`io/22_injection_nodim_validation.ipynb`): the 19.5–21.5 donor pool is compact galaxies (median concentration f(<0.3")/f(<1.5")≈0.31 vs ≈0.6 for the real faint population), so these numbers are extended-morphology completeness, not point-source depth. Point-source rerun with star-like donors (C≥0.65, pool 19.5–22.5, 3" stamps): **cn_vis_sep 26.59 all / 26.14 VIS / +0.45 gain**; cn_mer 26.34/26.06/+0.28; stem_mer 26.45/26.45/+0.0. Validation: real sources re-injected at native brightness match the star-dim curve at 23–25; empty-sky null 0.4%. Metrics `injection_metrics_r30_star{,_deep}.json`; paper Fig 6/Table 1 updated. Legacy run below kept for the record (final 108-tile run, `injection_metrics.json`, log `q1_injection_full.log` — this overwrote a preliminary 24-tile eval, so quotes of 24.88/24.43 are stale): cn_vis_sep 50% depth **25.04 all-band vs 24.59 VIS-only → fusion gain +0.45 mag**; cn_mer 25.1/24.7/+0.40; stem_mer +0.03 only — the fusion gain lives in the fused bottleneck, not the stems. NISP-only injections peak at **36.8% completeness** (never reach 50%). **Rubin-only ≈ 0 was NOT re-verified on Q1** — that claim rests on the DR1-era test (notebook `io/17` Test 3); the paper states it, flag if a referee pushes.
...
- **Partial-band (spatial) masking in pretraining**: v10 masking is whole-band only — the reconstruction target's input slot is zeroed for the entire crop, and the adversarial drop removes 1–2 additional whole context bands (`jaisp_dataset_v10.py`). Partially missing pixels get no special treatment: NaNs become 0 with MAD-fallback RMS, and the stems take a single noise-normalized channel with no validity mask, so a data gap inside a band looks like blank sky rather than absent data. Future direction (SH, 2026-07-13): also mask random spatial regions within a band during pretraining — irregular blobs (detector defects), streaks (cosmic-ray tracks), strips (chip gaps) — and optionally add a per-pixel validity channel to the stems so "no data" is distinguishable from "blank sky". This would turn the current whole-band robustness into learned inpainting over arbitrary coverage gaps, and fits the DR1 native-NISP direction where the model absorbs more upstream preprocessing. Note the paper's §3 claim ("lets the model operate when some bands are absent") was reviewed 2026-07-13 and deliberately refers to whole bands.
