...
3. **Joint 10-band canonical centroiding** was then truth-validated as a better canonical definition (2.4-2.5x lower floor at every SNR) and a patch-disjoint retrain on joint labels (`latent_position_v10_patchval25_joint`) is in progress — see "Joint canonical labels" below.
...
*Median position error vs injected truth (synthetic injection into held-out patch-25 tiles, Rubin bands). Red: classical single-band centroid (no DL). Brown dashed: the VIS-only canonical definition — the current head's training target and therefore its floor. Blue: the spatially held-out head — it reaches its floor plus 2-4 mas at every SNR, i.e. it transfers the VIS localization to every band. Green: the joint 10-band canonical definition validated 2026-06-11 (equal-SNR best case). Shaded band: projected accuracy of the head retrained on joint labels (run in progress). Gray dotted: the ~4-7 mas real concordance field for scale.*

##### Joint canonical labels (2026-06-11, in progress)
...
- **Running A/B**: `latent_position_v10_patchval25_joint` (patch 25 held out, all else identical to `latent_position_v10_patchval25`). Verdict via the injection harness: success = faint-end truth error entering the projected band (~9-10 mas at SNR 5). If it passes, joint labels become the production canonical convention and the production head + anchor exports get regenerated.
