...
**Precomputed features**: To make training fast, the frozen V7 encoder is run once on all 144 tiles (with 8 augmentation variants each: 4 rotations × 2 flips) and the bottleneck feature tensors `[256, ~130, ~130]` are saved to disk. Training then runs only the lightweight decoder neck + heads on cached features — no encoder forward pass needed per step.
...
The training data consists of 130 training tiles and 14 validation tiles with random 90-degree rotations and flips (8 variants cached per tile). All 10 bands (6 Rubin + 4 Euclid) are fed through the frozen V7 encoder during precomputation.
...
| V7 detector (CenterNet) | `checkpoints/centernet_v7_selftrain_vis/centernet_best.pt` | CenterNet with VIS-resolution decoder, self-trained with spike demotion |
| Cached features | `data/cached_features_v7/` | Precomputed V7 encoder bottleneck tensors (all tiles × 8 augs) |
